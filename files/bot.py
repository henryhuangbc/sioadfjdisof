			...
			for member in ctx.guild.members:
				if member == client.user:
					...
					try:
						...
						await ctx.send(f":white_check_mark: Sent message to {member.name}. HENRY PROCODER")
						...
						member_count -= 1
						pass
			await ctx.send(f":white_check_mark: DM sent to {member_count} members. YESS SIR")
			return
		else:
			member_count = 0
			for member in ctx.guild.members:
...
async def dm(ctx, user:MemberConverter=None, *, args=None):
	while args != None and args != "" and not args.isspace():
		try:
			...
			await ctx.send(f"Message sent to {user.name}#{user.discriminator} :white_check_mark:. HENRY PROCODER")
		except discord.errors.Forbidden:
...
async def help(ctx, command_name:str=None):
	while command_name == None or command_name.isspace() or command_name == "":
		...
		xd.set_footer(text="HENRYYYYYYYYYY", icon_url="https://i.ibb.co/VJLSFTH/standard-3.gif")
	...
	xdd.set_footer(text="Henryyyyy", icon_url="https://cdn.discordapp.com/avatars/559227438224375828/95f57511cebe80102e73a50eb892506e.webp?size=1024")
